...
# 🫧 웹퍼블리셔 지원용 포트폴리오 (Bubble Mate 프로젝트)

---

## 📌 Candidate Summary (지원자 포지셔닝)
- **지원 직무**: 웹 퍼블리셔 (Web Publisher) / UI 개발자
- **핵심 역량**: 
  - AI 도구(Claude 등)와 최신 워크플로우를 활용한 **초고속 웹 퍼블리싱 및 개발 생산성 극대화**
  - 개발자 도구(F12) 기반의 **정교한 CSS/JS 디버깅 및 디테일 레이아웃 보완 능력**
  - 280px(갤럭시 폴드)부터 4K 데스크톱까지 대응하는 **5단계 반응형 웹(Responsive Web) 구현**
  - 서비스 전체 구조와 사용자 동선을 이해하고 코드로 구현해내는 **기획 이해도 기반 퍼블리싱**

---

## 🛍️ MAIN PROJECT: Bubble Mate (프로페셔널 카케어 쇼핑몰)

> **"Vanilla JS와 Pure CSS 기반의 5단계 반응형 웹 및 풀스택 UI/UX 퍼블리싱"**

### 1. 프로젝트 개요
- **프로젝트명**: Bubble Mate (버블메이트)
- **프로젝트 성격**: 프리미엄 세차 용품 전문 온라인 쇼핑몰 및 관리자 시스템
- **주요 URL**:
  - **쇼핑몰 라이브 배포**: [Bubble Mate 메인 페이지](https://wlsgns9607-blip.github.io/Bubble_Mate/)
  - **관리자 대시보드**: [Admin Dashboard](https://wlsgns9607-blip.github.io/Bubble_Mate/admin.html)
  - **GitHub 레포지토리**: [GitHub Repository](https://github.com/wlsgns9607-blip/Bubble_Mate)
- **작업 기여도**: 기획 및 UI/UX 설계 100%, 웹 퍼블리싱 및 프론트엔드 구현 100%
- **사용 기술**: `HTML5`, `CSS3 (Flexbox/Grid)`, `JavaScript (ES6+)`, `Chart.js`, `Toss Payments API`, `OAuth 2.0`

---

### 2. 핵심 퍼블리싱 & UI/UX 구현 스펙

#### 🎨 1) 일관된 Design System 및 마이크로 인터랙션
- Root CSS 변수(`--blue`, `--navy`, `--mint` 등)를 정의하여 전체 사이트의 컬러 파렛트 및 테마 일관성 유지
- CSS Keyframes 애니메이션을 활용한 로고 버블 모션, 컨페티(Confetti) 효과, 선물상자 바운스 등 생동감 있는 마이크로 인터랙션 제공
- CSS Flexbox 및 Grid 구조를 조합하여 제품 카드의 규칙적인 정렬과 반응형 유연성 확보

#### 📱 2) 5단계 브레이크포인트 기반 초디테일 반응형 웹
- **극소형 기기 대응**: 갤럭시 폴드 접힘 상태(280px~300px)에서도 레이아웃이 깨지지 않는 모바일 최적화 퍼블리싱
- **모바일 특화 UI**:
  - **Option Sheet**: 구매하기 클릭 시 모바일 하단에서 부드럽게 올라오는 Slide-up 옵션 선택 창
  - **Cart Drawer**: 우측에서 슬라이딩되는 장바구니 드로어 레이아웃 및 실시간 뱃지 동기화
  - **Sticky Buy Bar**: 상세 페이지 스크롤 시 하단에 고정되는 모바일 전용 구매 바

#### ⚡ 3) 사용자 경험(UX) 개선을 위한 고급 퍼블리싱 기법
- **스켈레톤 UI (Skeleton Loader)**: 상세 페이지 진입 시 비동기 로딩 구간 동안 뼈대 레이아웃을 노출하여 사용자 이탈 방지
- **무한 루프 슬라이더**: 이벤트 배너 연출 시 복제 카드 트릭과 `scroll-behavior` 제어를 통해 끊김 없는 부드러운 무한 스크롤 구현
- **상태 관리 연동**: LocalStorage 기반의 장바구니 데이터 단방향 흐름 구조 설계 및 실시간 UI 반영

#### 📊 4) 관리자(Admin) 대시보드 퍼블리싱 & 데이터 시각화
- 대시보드 통계 카드, 최근 주문 내역 테이블, 상품 CRUD 모달창 퍼블리싱
- `Chart.js` 연동을 통한 주간 일별 매출 추이 라인 차트 컴포넌트 구현
- 세션 기반의 관리자 로그인 보호 레이어 및 모달 시스템 연동

---

### 3. AI 시대 맞춤형 개발 워크플로우 (AI + Debugging)

```
[서비스/UI 기획] ──> [AI 프롬프팅으로 80% 뼈대 생성] ──> [F12 디버깅 & CSS/JS 디테일 보완] ──> [완성 및 배포]
```

- **생산성 극대화**: AI 도구(Claude 등)를 활용하여 시맨틱 HTML 구조와 CSS 기본 프레임을 빠른 속도로 도출
- **정교한 오류 수정**:
  - 모바일 해상도별 padding/margin 오차 디버깅
  - Toss Payments 결제 모듈 호출 시 모달과의 `z-index` 레이어 충돌 문제 해결
  - OAuth 2.0 (Google/Naver/Kakao) 인증 리다이렉트 URI 동적 생성 로직 보완
...
├── admin.html          # 관리자 대시보드
├── style.css           # 메인 스타일시트 (5단계 반응형 포함)
├── admin.css           # 관리자 페이지 전용 스타일시트
├── app.js              # 쇼핑몰 인터랙션 & 데이터 로직 (1,400+ lines)
├── products.js         # 상품 및 리뷰 데이터 구조
├── admin.js            # 관리자 대시보드 CRUD & Chart.js 연동
└── README.md           # 프로젝트 문서
```
...
**Q. 프로젝트 구현 과정에서 AI를 적극적으로 활용하였나요?**
> "네, 저는 최신 IT 환경에 맞춰 AI 도구를 보조 개발자로 적극 활용했습니다. AI를 통해 기본 코드 작성 시간을 80% 이상 단축시켰으며, AI가 놓치는 크로스 브라우징 오차, 모바일 스티키 바 디테일, 결제 모듈 레이어 충돌 등은 제가 직접 브라우저 개발자 도구(F12)로 CSS와 JS 소스를 분석하여 디버깅해냈습니다. AI를 활용해 압도적인 생산성으로 고품질의 결과물을 빠르게 뽑아내는 것이 저의 핵심 경쟁력입니다."
---
